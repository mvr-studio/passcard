...
export const useWallets = ({ setAreWalletsLoading, allowedBlockchains }: UseWalletsProps) => {
  ...
  const allowsCardano = useMemo(() => allowedBlockchains.includes(Blockchain.Cardano), [allowedBlockchains])
  const allowsMina = useMemo(() => allowedBlockchains.includes(Blockchain.Mina), [allowedBlockchains])
  const allowsEthereum = useMemo(() => allowedBlockchains.includes(Blockchain.Ethereum), [allowedBlockchains])
  const allowsSolana = useMemo(() => allowedBlockchains.includes(Blockchain.Solana), [allowedBlockchains])

  const cardanoWallets = useMemo(
    () => caradanoWalletsRaw.map((wallet) => ({ ...wallet, blockchain: Blockchain.Cardano })),
    [caradanoWalletsRaw]
  )
